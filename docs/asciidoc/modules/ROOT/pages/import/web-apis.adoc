...
== Load Single File From Archived File (zip/tar/tar.gz/tgz)

When loading data from archived files, we need to put the `!` character before the file name or path in the compressed file. If the file is archived along with a compression algo, e.g. a `tar.gz` or a `tgz` we have to use the config `compressionAlgo` to decompress the archive file.
For example:

.Loading an archived JSON file
----
apoc.load.json("https://github.com/neo4j/apoc/tree/3.4/src/test/resources/testload.zip?raw=true!person.json");
----

.Loading an archived and compressed JSON file
----
apoc.load.json("path/to/file/myFile.tar.gz!person.json");
----
...
We can download it from https://github.com/neo4j/apoc/releases/download/{apoc-release}/apoc-redis-dependencies-{apoc-release}.jar[this link] or locally downloading the apoc repository:
----
